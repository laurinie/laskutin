...
export function refWithCheck(body: string | number): string | null {
  ...
  if (digits.length < MIN_BODY_DIGITS || digits.length > MAX_BODY_DIGITS) return null;
...
export function referenceFor(index: number, opts: ReferenceOptions): string {
  if (opts.mode === 'shared') return sharedReference(opts.shared);
...
function sharedReference(given: string): string {
...
export const ibanPrettyOrEmpty = (s: string): string => (s.trim() ? ibanPretty(s) : '');
