/* Jäsenlistan jäsentäminen tekstistä tai taulukosta. */
...
const HEADER_PATTERNS: ReadonlyArray<[keyof ColumnMap, RegExp]> = [
...
export function rowToMember(cols: string[], map: ColumnMap | null): Member {
  if (map) {
    const pick = (i: number | undefined): string => (i === undefined ? '' : String(cols[i] ?? '').trim());
...
export interface ParsedMembers {
  members: Member[];
  /** Tunnistetut sarakkeet käyttäjälle näytettäväksi. */
  columnInfo: string;
}
...
export function itemsFor(member: Member, base: LineItem[], fallbackDesc: string): LineItem[] {
