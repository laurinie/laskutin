...
function el(id) {
    ...
    if (!node)
        throw new Error(`Elementtiä #${id} ei löytynyt`);
    return node;
}
...
function invoiceFor(member, index) {
...
function renderTable() {
    ...
    const parts = [];
    if (columnInfo)
        parts.push(`Tunnistetut sarakkeet → ${columnInfo}`);
    if (bad)
        parts.push(`${bad} riviltä puuttuu kelvollinen sähköposti`);
...
function validate() {
    ...
    if (!iban) {
        ib.textContent = 'IBAN puuttuu.';
        ib.className = 'check warn';
    }
    else if (ibanValid(iban)) {
        ib.textContent = `✓ IBAN kelvollinen: ${ibanPretty(iban)}`;
        ib.className = 'check ok';
    }
    else {
        ib.textContent = '✗ IBAN ei läpäise tarkistusta – tarkista numero.';
        ib.className = 'check err';
    }
    const rc = el('refCheck');
    if (refMode() === 'shared') {
        const raw = val('sharedRef').replace(/\D/g, '');
        if (!raw) {
            rc.textContent = 'Anna yhteinen viite.';
            rc.className = 'check warn';
        }
        else if (refIsValid(raw)) {
            rc.textContent = `✓ Viite ${refPretty(raw)} on kelvollinen.`;
            rc.className = 'check ok';
        }
        else {
...
function newDoc() {
    ...
    if (!lib)
        throw new Error('jsPDF ei latautunut – tarkista että vendor/jspdf.umd.min.js on paikallaan.');
    return new lib.jsPDF({ unit: 'mm', format: 'a4', compress: true });
}
function guard() {
    if (!members.length) {
        setStatus('Lisää ensin jäsenlista.', true);
        return false;
    }
    if (!window.jspdf) {
        setStatus('jsPDF ei latautunut – tarkista vendor-hakemisto.', true);
        return false;
    }
    return true;
}
...
async function zipPdfs() {
    if (!guard())
        return;
    if (!window.JSZip) {
        setStatus('JSZip ei latautunut – tarkista vendor-hakemisto.', true);
        return;
    }
    const btn = el('zipBtn');
    btn.disabled = true;
...
async function importFile(file) {
    try {
        if (/\.xlsx$/i.test(file.name) || /spreadsheetml/.test(file.type)) {
            if (!window.JSZip)
                throw new Error('JSZip ei latautunut – Excel-tuonti ei käytettävissä.');
            const rows = await readXlsx(file, window.JSZip);
            if (!rows.length) {
                setStatus('Excel-tiedostosta ei löytynyt rivejä.', true);
                return;
            }
...
function save() {
    ...
    if (logo && logo.dataUrl.length < 1500000)
        data.logo = logo;
    try {
...
function restore() {
    ...
    if (data.refMode) {
        const radio = document.querySelector(`input[name=refMode][value="${data.refMode}"]`);
        if (radio)
            radio.checked = true;
    }
    if (data.logo) {
...
function init() {
    ...
    el('tplXlsxBtn').onclick = () => {
        void (async () => {
            try {
                if (!window.JSZip)
                    throw new Error('JSZip ei latautunut – Excel-pohjaa ei voi luoda.');
    ...
    document.querySelectorAll('input[name=refMode]').forEach((radio) => {
