...
import { money, parseAmount, fmtDate, isEmail, slug, csvField } from '../dist/format.js';
import { refWithCheck, refIsValid, refPretty, referenceFor, ibanValid, ibanPretty } from '../dist/reference.js';
import { parseMembers, parseLineItems, itemsFor, splitCols } from '../dist/members.js';
...
test('viitteet per jäsen ja yhteisenä', () => {
  const per = { mode: 'per', prefix: '2026', start: 1, shared: '' };
  assert.equal(referenceFor(0, per), '202600017');
  assert.notEqual(referenceFor(0, per), referenceFor(1, per));
  const shared = { mode: 'shared', prefix: '', start: 1, shared: '2026 0001' };
...
test('laskurivit ja jäsenkohtainen summa', () => {
  ...
  const withoutOwn = itemsFor({ name: 'A', email: 'a@b.fi', amount: null, badEmail: false }, base, 'Lasku');
  assert.equal(withoutOwn.length, 2, 'ilman omaa summaa käytetään oletusrivejä');

  const withOwn = itemsFor({ name: 'B', email: 'b@b.fi', amount: 20, badEmail: false }, base, 'Lasku');
